...
////////////////////////////////////////////////////////////////////////////////
// RTC_PCF85063 implementation

boolean RTC_PCF85063::begin(void) {
  Wire.begin();
  return true;
}

boolean RTC_PCF85063::initialized(void) {
  Wire.beginTransmission(PCF85063_ADDRESS);
  Wire._I2C_WRITE((byte)PCF85063_CONTROL_1);
  Wire.endTransmission();
...
void RTC_PCF85063::adjust(const DateTime& dt) {
  Wire.beginTransmission(PCF85063_ADDRESS);
  Wire._I2C_WRITE((byte)4); // start at register 4
  Wire._I2C_WRITE(bin2bcd(dt.second()));
  Wire._I2C_WRITE(bin2bcd(dt.minute()));
  Wire._I2C_WRITE(bin2bcd(dt.hour()));
  Wire._I2C_WRITE(bin2bcd(dt.day()));
...
DateTime RTC_PCF85063::now() {
  Wire.beginTransmission(PCF85063_ADDRESS);
  Wire._I2C_WRITE((byte)4);
  Wire.endTransmission();

  Wire.requestFrom(PCF85063_ADDRESS, 7);
  uint8_t ss = bcd2bin(Wire._I2C_READ() & 0x7F);
  uint8_t mm = bcd2bin(Wire._I2C_READ());
  uint8_t hh = bcd2bin(Wire._I2C_READ());
  uint8_t d = bcd2bin(Wire._I2C_READ());
...
Pcf8523SqwPinMode RTC_PCF85063::readSqwPinMode() {
  int mode;

  Wire.beginTransmission(PCF85063_ADDRESS);
  Wire._I2C_WRITE(PCF85063_CONTROL_2);
  Wire.endTransmission();

  Wire.requestFrom((uint8_t)PCF85063_ADDRESS, (uint8_t)1);
  mode = Wire._I2C_READ();
...
void RTC_PCF85063::writeSqwPinMode(Pcf8523SqwPinMode mode) {
  Wire.beginTransmission(PCF85063_ADDRESS);
  Wire._I2C_WRITE(PCF85063_CONTROL_2);
  Wire._I2C_WRITE(mode);
  Wire.endTransmission();
...
void selectCapacitor(Pcf85063Capacitor capacitor) {
  Wire.beginTransmission(PCF85063_ADDRESS);
  Wire._I2C_WRITE(PCF85063_CONTROL_1);
  Wire._I2C_WRITE(capacitor);
  Wire.endTransmission();
}
